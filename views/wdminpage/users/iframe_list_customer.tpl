...
<div class="pd15" ng-controller="userListController" ng-app="ngApp">
                    ...
                    <div class="col-xs-9">
                        <div class="input-group">
                            <div class="input-group-btn">
                                <button type="button" style="line-height: 20px;" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span id="search-type-label">电话</span><span class="caret" style="margin-left: 5px;"></span></button>
    ...
    <table class="table table-hover table-bordered" style="margin-bottom: 50px;table-layout: fixed">
        <thead>
        <tr>
            <th width="50px">编号</th>
            ...
            <th>电话</th>
            <th width="50px" class="text-center">性别</th>
            <th>积分</th>
            <th>分组</th>
            <th width="80px" class="text-center">注册日期</th>
        ...
        <tr ng-repeat="usr in userlist" class="usrlist">
            <td>{{usr.client_id}}</td>
            <td style="width: 53px;" class="text-center"><img class="ccl-head" src="{{usr.client_head}}"/></td>
            <td>{{usr.client_name}}</td>
            <td>{{usr.client_province}}{{usr.client_city}}</td>
            <td>{{usr.client_phone}}</td>
            <td class="text-center">{{sexStr[usr.client_sex]}}</td>
            <td>{{usr.client_credit}}</td>
            <td>{{userLevelStr[usr.client_level]}}</td>
            <td>{{usr.client_joindate}}</td>
            <td>
                <a class="text-success" data-toggle="modal" data-target="#modal_modify_user" data-id="{{usr.client_id}}" href="#">编辑</a>
