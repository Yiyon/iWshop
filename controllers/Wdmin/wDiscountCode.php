...
class wDiscountCode extends Controller {
    ...
    public function upload($Q) {
        ...
        if (!empty($_FILES)) {
            ...
            while (!feof($files)) {
                ...
                $this->Dao->insert($this->table, 'codes,qid')
                          ->values(array(
                              trim($code),
                              $qid
                          ))
    ...
    public function alterCodes($Q) {
        ...
        if ($id > 0) {
            // alter
            $fg = $this->Dao->update($this->table2)
                            ->set(array(
                                'keywords' => $keywords,
        ...
        if (!$id) {
            // create
            $fg = $this->Dao->insert($this->table2, 'keywords, code_discount, `template`')
                            ->values(array(
                                $keywords,
                                $discount,
                                $template
    ...
    public function alterCode($Q) {
        ...
        if (!$codes && $id > 0) {
            ...
            if ($id > 0) {
                // alter
                $fg = $this->Dao->update($this->table)
                                ->set(array('codes' => $codes))
                                ->where('id', $id)
                                ->exec();
            }
            if ($id < 0) {
            ...
            if (!$id) {
                // create
                $fg = $this->Dao->insert($this->table, 'codes, qid')
                                ->values(array(
                                    $codes,
                                    $qid
                                ))
