...
//加载RequestCore
require_once OSS_API_PATH . '/lib/requestcore/requestcore.class.php';
...
class ALIOSS{
	...
	public function auth($options){
		...
		//创建请求
		$request = new OSS_RequestCore($this->request_url);
		...
		$data =  new OSS_ResponseCore ( $response_header , $request->get_response_body (), $request->get_response_code () );
	...
	public function complete_multipart_upload($bucket, $object, $upload_id, $parts, $options = NULL){
		...
		if(is_string($parts)){
		...
		}else if((is_array($parts) || $parts instanceof OSS_ResponseCore)){
			...
			}elseif ($parts instanceof OSS_ResponseCore){
