...
class WdminAjax extends Controller {
    ...
    public function alterGmessPage() {
        ...
        if (is_file(APP_PATH . 'uploads/gmess_tmp/' . $this->post('catimg'))) {
            @rename(APP_PATH . 'uploads/gmess_tmp/' . $this->post('catimg'), APP_PATH . 'uploads/gmess/' . $this->post('catimg'));
        }

        if ($msgId > 0) {
            $oldData = $this->mGmess->getGmess($msgId);
            if ($oldData['catimg'] != $this->post('catimg')) {
                @unlink(APP_PATH . 'uploads/gmess/' . $oldData['catimg']);
            }
            ...
            #$this->Dao->echoSql();
            $this->echoJson(array('status' => $rst ? 1 : 0));
        } else {
            ...
            if ($rst) {
                $ret['status'] = 1;
                $ret['url']    = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://" . $this->server('HTTP_HOST') . "$config->shoproot?/Gmess/view/id=" . $rst;
                $ret['msgid']  = $rst;
            } else {
                $ret['status'] = 0;
            }
	...
	*, cl.client_id AS cid,
    ...
    public function EditorImageUpload(){
        ...
        $this->loadModel('ImageUploader');
        $this->ImageUploader->dir = $config->oss? $config->ossDir.'/ueditor/' :APP_PATH . 'uploads/ueditor/';
        $file                     = $this->ImageUploader->upload($config->oss,$config->access_id,$config->access_key,$config->bucket);
        
        $info = array(
            "originalName" => $_FILES[$keyName]['name'] ,
            "name" => $file ,
            "url" => $file?$config->oss? $file :'uploads/ueditor/' . $file:'' ,
