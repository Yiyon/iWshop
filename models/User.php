...
class User extends Model {
     ...
     * @return boolean
     */
    public function setCredit($uid, $credit, $remark = '') {
    ...
    public function mantUserBalance($amount, $uid, $type = self::MANT_BALANCE_ADD) {
        ...
        if ($this->Db->query($mSql) !== false) {
            ...
            return $this->Dao->insert('client_money_record', `client_id`, `amount`, `time`)
                             ->values(array(
                                 $uid,
                                 $amount,
                                 'NOW()'
    ...
    public function wechatAutoReg($openid = '') {
        global $config;
        // 检查用户是否注册
        if (Controller::inWechat() && !$this->userCheckReg($openid)) {
            ...
            if ($uid !== false) {
                ...
                if ($ret) {
                    ...
                    if ($comid > 0) {
                        // 更新代理对应
                        $ret = $this->Dao->update(TABLE_USER)
                ...
                $this->sCookie('uopenid', $openid, Uc::COOKIEXP);
    ...
    public function getDiscount($uid) {
        if ($uid > 0) {
            return $this->Dao->select('level_discount')
                             ->from(TABLE_USER)
                             ->alias('us')
