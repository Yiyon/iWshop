...
class WdminPage extends Controller {
    ...
    public function product_share_qrcode($Query) {
        if (is_numeric($Query->id)) {
            ...
            if (!$this->isCached()) {
                $this->loadModel('Product');
                $this->assign('qrcode', 'http://api.ycchen.cn/qrcode/generate/?url=' . urlencode($this->Product->getURI($Query->id, $this->pCookie('comid') ? $this->Util->digDecrypt($this->pCookie('comid')) : false)));
            }
    ...
    public function list_customer_address($Query) {
        ...
        $this->show(self::TPL . 'users/list_customer_address.tpl');
    }
    ...
    public function settings_sign() {
        ...
        $this->show(self::TPL . 'settings/settings_credit.tpl');
    }
