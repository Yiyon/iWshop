...
class OSS_RequestCore
{
	 ...
	 */
	public $request_class = 'OSS_RequestCore';

	/**
	 * The default class to use for HTTP Responses (defaults to <ResponseCore>).
	 */
	public $response_class = 'OSS_ResponseCore';
	...
	public function streaming_read_callback($curl_handle, $file_handle, $length)
	{
		...
		if ($this->read_stream_read == 0 && isset($this->seek_position) && $this->seek_position !== ftell($this->read_stream))
		{
			...
			{
				throw new OSS_RequestCore_Exception('The stream does not support seeking and is either not at the requested position or the position is unknown.');
			}
		}
...
//			curl_setopt($curl_handle, CURLOPT_SSL_VERIFYHOST, true);
		...
		switch ($this->method)
		{
			case self::HTTP_PUT:
				...
				if (isset($this->read_stream))
				{
					...
					{
						throw new OSS_RequestCore_Exception('The stream size for the streaming upload cannot be determined.');
					}
	...
	public function send_request($parse = false)
	{
		...
		{
			throw new OSS_RequestCore_Exception('cURL resource: ' . (string) $curl_handle . '; cURL error: ' . curl_error($curl_handle) . ' (' . curl_errno($curl_handle) . ')');
		}
	...
	public function send_multi_request($handles, $opt = null)
	{
		...
		do
		{
			...
			while ($done = curl_multi_info_read($multi_handle))
			{
				...
				if ($done['result'] > 0)
				{
					throw new OSS_RequestCore_Exception('cURL resource: ' . (string) $done['handle'] . '; cURL error: ' . curl_error($done['handle']) . ' (' . $done['result'] . ')');
				}
 ...
 */
class OSS_RequestCore_Exception extends Exception {}
