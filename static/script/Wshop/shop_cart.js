...
$(function(){
    $("#select-address li").bind('click',function(){
        var selectIndex = $(this).index();
        $('.select-icon').removeClass('selected-address');
        $(this).children('.select-icon').addClass('selected-address');
        if(selectIndex == 1){
            document.getElementById('take-selected').disabled = true;
            $('.take-oneself-bar').addClass('take-oneself-bar-disable');
            document.getElementById('take-show').innerHTML = '点击选择自提地址';
        }else {
            document.getElementById('take-selected').disabled = false;
            $('.take-oneself-bar').removeClass('take-oneself-bar-disable');
            document.getElementById("take-selected").value = 0;
        }
    });
    document.getElementById('take-selected').addEventListener('change',function(){
        var takeValue = document.getElementById("take-selected").value;
        document.getElementById('take-show').innerHTML = takeValue;
    });
});