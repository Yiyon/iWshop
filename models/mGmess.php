...
class mGmess extends Model {
    ...
    public function alterGmess($msgId, $title, $content, $desc, $catImg, $thumbMediaId = null, $category = 0) {
        if ($msgId > 0) {
            ...
            // 插入数据
            return $this->Dao->insert(TABLE_GMESS, 'title, content, `desc`, catimg, createtime, category, thumb_media_id')
                            ->values(array($title, $content, $desc, $catImg, date("Y-m-d"), $category, $thumbMediaId))->exec();
        }
    }
    ...
    public function getGmessList() {
        ...
        foreach ($list as &$l) {
            $l['href'] = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://" . $this->server('HTTP_HOST') . "$config->shoproot?/Gmess/view/id=" . $l['id'];
        }
        return $list;
    }
