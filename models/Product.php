        ...
        try {
            ...
            if ($id == 'NULL') {
                ...
                $this->catImg = $this->Dao->select('catimg')
                                          ...
                                          ->where("product_id = $id")
                                          ->getOne();
                $ret          = $this->Db->query($packedData->sql);
            }
    ...
    public function handleProductImages($id, $images) {
        ...
        foreach ($diff as $sort => $image) {
            ...
            if (preg_match("/$this->imageUploadMark/is", $image)) {
                ...
                if ($sort != -1) {
                    $this->Dao->insert('product_images', '`product_id`,`image_path`,`image_type`,`image_sort`')
                              ->values(array(
                                  $id,
                                  $image,
                                  0,
