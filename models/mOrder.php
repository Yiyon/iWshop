...
class mOrder extends Model {
    ...
    public function create($openid, $orderlist, $address, $params = []) {
        ...
        try {
            ...
            // 写入订单数据
            $orderFields = [
                'status', 'company_com', 'client_id', 'product_count', 'order_balance', 'original_amount', 'order_amount', 'order_yunfei', 'order_time', 'wepay_serial', 'envs_id', 'wepay_openid', 'leword', 'serial_number', 'exptime'
            ];
            $orderValues = [
                $orderStatus, $companyCom, $this->uid, $this->order_product_count, $orderBalance, $originalAmount, $orderAmount, $params['expfee'], 'NOW()', '', $params['envsid'], $openid, $params['remark'], $serial_number, $params['exptime']
            ];
            $orderId     = $this->Dao->insert(TABLE_ORDERS, $orderFields)
                                     ->values($orderValues)
                                     ->exec();
            // 生成订单详情信息
    ...
    public function wechat_deliverNotify($orderId) {
        ...
        if (!empty($data['wepay_serial']) && !empty($data['wepay_openid'])) {
            ...
            $postData      = array(
                "appid" => APPID, "openid" => $data['wepay_openid'], "transid" => $data['wepay_serial'], "out_trade_no" => $orderId, "deliver_timestamp" => $dtime, "deliver_status" => "1", "deliver_msg" => "ok", "app_signature" => $app_signature, "sign_method" => SIGNTYPE
            );
            $curl          = curl_init();
        ...
        if ($id > 0) {
            ...
            foreach ($orderData['products'] as &$pds) {
                ...
                $pds   = array_merge(array(
                    'phid' => $pds['product_price_hash_id'], 'product_count' => $pds['product_count'], 'product_discount_price' => $pds['product_discount_price'], 'refunded' => intval($pds['refunded']), 'detail_id' => intval($pds['detail_id'])
                ), $pinfo);
            }
            return $orderData;
        } else {
        ...
        if (!empty($tpl['tpl_id'])) {
            ...
            foreach ($openIds as $openid) {
                ...
                Messager::sendTemplateMessage($tpl['tpl_id'], $openid, array(
                    $tpl['first_key'] => '有一位顾客下单了，请尽快发货', $tpl['serial_key'] => $orderInfo['serial_number'], $tpl['product_name_key'] => implode('、', $orderInfos), $tpl['product_count_key'] => $orderInfo['product_count'] . '件', $tpl['order_amount_key'] => '¥' . sprintf('%.2f', $orderInfo['order_amount']), $tpl['remark_key'] => '点击详情 随时查看订单状态'
    ...
    public function userNewOrderNotify($orderId, $openid) {
        ...
        if (!empty($tpl['tpl_id'])) {
            ...
            return Messager::sendTemplateMessage($tpl['tpl_id'], $openid, array(
                $tpl['first_key'] => '感谢您在' . $config->shopName . '购物', $tpl['serial_key'] => $orderInfo['serial_number'], $tpl['product_name_key'] => implode('、', $orderInfos), $tpl['product_count_key'] => $orderInfo['product_count'] . '件', $tpl['order_amount_key'] => '¥' . sprintf('%.2f', $orderInfo['order_amount']), $tpl['remark_key'] => '点击详情 随时查看订单状态'
            ), $this->getBaseURI() . "?/Order/expressDetail/order_id=$orderId");
        }
    }
    ...
    public function orderRefund($orderId, $refund_fee = false) {
        ...
        if ($orderId > 0) {
            ...
            if ($refund_fee > $totalFee) {
                ...
                $postData = array(
                    "appid" => APPID, "mch_id" => PARTNER, "transaction_id" => $orderInfo['wepay_serial'], "out_trade_no" => $config->out_trade_no_prefix . $orderId, "out_refund_no" => $orderId . $refund_fee, "total_fee" => $totalFee, "refund_fee" => $refund_fee, "op_user_id" => PARTNER, "nonce_str" => $this->createNoncestr()
                );
    ...
    public function updateOrderStatus($orderId, $status, $refundAmount = false) {
        if ($refundAmount) {
            return $this->Dao->update(TABLE_ORDERS)
                             ->set(array(
                                 'status' => $status, 'order_refund_amount' => $refundAmount
                             ))
                             ->where(['order_id' => $orderId])
                             ->exec();
        } else {
    ...
    public function addComment($openid, $orderid, $content, $stars) {
        ...
        if ($this->User->checkUserExt($openid) && $orderid > 0 && is_numeric($stars)) {
            $ret = $this->Dao->insert(TABLE_ORDERS_COMMENT, 'openid,starts,content,mtime,orderid')
                             ->values(array(
                                 $openid, $stars, $content, Dao::FIELD_NOW, $orderid
                             ))
                             ->exec();
            if ($ret) {
                return $this->Dao->update(TABLE_ORDERS)
    ...
    public function logRefundment($order_id, $serial_number, $refund_amount, $refund_serial, $pay_type, $type = 0) {
        return $this->Dao->insert(TABLE_REFUNDMENT, [
            'order_id', 'serial_number', 'refund_amount', 'refund_time', 'refund_type', 'refund_serial', 'payment_type'
        ])
                         ->values([
                             $order_id, $serial_number, $refund_amount, 'NOW()', $type, $refund_serial, $pay_type
                         ])
                         ->exec();
    }
