...
class StatOverView extends Model {
    ...
    public function getOverViewDatas() {
        ...
        // 今日成交
        $data['saletoday'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE DATE_FORMAT(`order_time`,'%Y-%m-%d') = '$QueryDay' AND `wepay_serial` <> '';");
        // 昨日成交
        $data['saleyestoday'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE DATE_FORMAT(`order_time`,'%Y-%m-%d') = '$QueryYestoDay' AND `wepay_serial` <> '';");
        // 本月成交
        $data['salemonth'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE DATE_FORMAT(`order_time`,'%Y-%m') = '$QueryMonth' AND `wepay_serial` <> '';");
        // 总成交
        $data['saletotal'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE `wepay_serial` <> '';");
        // 代理今日成交
        $data['pxsaletoday'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE DATE_FORMAT(`order_time`,'%Y-%m-%d') = '$QueryDay' AND `wepay_serial` <> '' AND `company_com` <> 0;");
        // 代理昨日成交
        $data['pxsaleyestoday'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE DATE_FORMAT(`order_time`,'%Y-%m-%d') = '$QueryYestoDay' AND `wepay_serial` <> '' AND `company_com` <> 0;");
        // 代理本月成交
        $data['pxsalemonth'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE DATE_FORMAT(`order_time`,'%Y-%m') = '$QueryMonth' AND `wepay_serial` <> '' AND `company_com` <> 0;");
        // 代理总成交
        $data['pxsaletotal'] = (float)$this->Db->getOne("SELECT SUM(order_amount) AS `sc` FROM `orders` WHERE `wepay_serial` <> '' AND `company_com` <> 0;");
        // 今日新增订单
