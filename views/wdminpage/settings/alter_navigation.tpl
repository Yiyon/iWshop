...
<form style="padding:15px 20px;padding-bottom: 70px;" id="settingFrom">
    ...
    <div class="fv2Field clearfix">
        ...
        <div class="fv2Right">
            <input type="text" class="gs-input" id="nav_name" value="{$sec.nav_name}" placeholder="请输入导航名称" autofocus/>
	...
	<div class="fv2Field clearfix">
        ...
        <div class="fv2Right">
            <div style="width:300px;margin-bottom: 20px;" id="radios">
                <input type="radio" id="rad1" name="actype" value="" {if $sec.nav_type == 1 || $sec.nav_type ==''}checked{/if}/>
                <label style="margin-right:5px;" onclick="$('#rad1').click();">产品分类</label>
                <input type="radio" id="rad2" name="actype" value=""  {if $sec.nav_type == 0 && $sec.nav_type !='' }checked{/if}/>
					...
					<div style="width:400px;margin-bottom: 20px;" >
                        <select id="pd-cat-select" style="color:#666">
							{foreach from=$categorys item=cat1}
								<option value="{$cat1.dataId}" {if $sec.nav_content eq $cat1.dataId}selected{/if}>{$cat1.name}</option>
								{foreach from=$cat1.children item=cat2}
									<option value="{$cat2.dataId}" {if $sec.nav_content eq $cat2.dataId}selected{/if}>-- {$cat2.name}</option>
									{foreach from=$cat2.children item=cat3}
										<option value="{$cat3.dataId}" {if $sec.nav_content eq $cat3.dataId}selected{/if}>---- {$cat3.name}</option>
									{/foreach}
								{/foreach}
							{/foreach}
						</select>
                ...
                <div id="act2" class="acts" style="{if $sec.nav_type == 1 || $sec.nav_type == ''}display:none;{/if}">
                    ...
                    <div style="width:400px;margin-bottom: 20px;">
                        <div class="gs-text">
                            <input type="text" value="{$sec.nav_content}" id="menu-url-ed" placeholder="输入网址需完整加上'http://'或'https://'"/>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="fv2Field clearfix">
        ...
        <div class="fv2Right">
            <input class='gs-input' type="text" name="sort" id='sort' onclick="this.select()" value="{$sec.sort}" />
        ...
        <div class="fv2Right">
            <div class="clearfix">
                <div class="alter-cat-img">
                    ...
                    <img id="catimage" src="{if $sec.nav_ico neq ''}{if $config.oss}{$sec.nav_ico}{else}{$docroot}uploads/navigation/{$sec.nav_ico}{/if}{/if}" />
                    {if $sec.nav_ico eq ''}
