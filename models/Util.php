...
class Util extends Model {
    ...
    public function paySign($pack) {
        ...
        file_put_contents('paySign.txt',$string);
        $string = md5($string);
    ...
    public static function log($message, $type = self::LOG_ERRORS) {
        ...
        $logdir .= date('Y-m') . DIRECTORY_SEPARATOR;
        if (!is_dir($logdir)) {
            mkdir($logdir, 0777, true);
        }
        if (!is_writable($logdir)) {
            chmod($logdir, 0777, true);
        }
        @error_log(date('Y-m-d H:i:s') . ': ' . $message . PHP_EOL, 3, $logdir . $type . '_log_' . date('Y-m-d') . '.log');
    }
